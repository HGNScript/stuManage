...
class tch extends Model
{
    ...
    public function notice($classtch_id, $admin_authority, $notice_flag)
    {
        ...
        if ($notice_flag == 1) {
            if ($admin_authority == 100) {
                $leave = (new student())->alias('s')
                    ->join('leave l', 's.stu_id = l.stu_id')
                    ->where('l.leave_flag', 1)
                    ->where('l.leave_flag_top', '=', 4)
                    ->select();
    ...
    public function classNotice($class_id, $leave_flag_top)
    {
